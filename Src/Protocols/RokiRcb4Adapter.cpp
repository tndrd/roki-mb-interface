...
RokiRcb4Adapter::RokiRcb4Adapter(MbInterface::Motherboard &mb) : RokiRcb4{mb} {}

bool RokiRcb4Adapter::setServoPos(std::vector<ServoData> &servoDatas,
                                  byte frame) {
  return RokiRcb4::setServoPos(servoDatas.data(), servoDatas.size(), frame);
}

bool RokiRcb4Adapter::setServoPosAsync(std::vector<ServoData> &servoDatas,
                                       byte frame, byte pause) {
...
std::tuple<bool, std::vector<byte>>
RokiRcb4Adapter::moveRamToComCmdSynchronize(int addr, byte size) {
